Component({
  ...
  data: {
    ...
    _img_top: (wx.getSystemInfoSync().windowHeight - Math.round(100 / 750 * wx.getSystemInfoSync().windowWidth)) / 2, //图片上边距
  ...
  attached() {
    ...
    this.data.info.windowHeight = this.data.info.windowHeight - Math.round(376 / 750 * this.data.info.windowWidth);
    //启用数据监听
    this._watcher();
  ...
  methods: {
    ...
    getImg(getCallback) {
      ...
      const destWidth = tp == 4 ? 2100 : (tp == 3 ? 1270 : (tp == 2 ? 1020 : 890));
      const destHeight = tp == 4 ? 2970 : (tp == 3 ? 1780 : (tp == 2 ? 1520 : 1270));
      ...
      this._draw(() => {
        wx.canvasToTempFilePath({
          ...
          width: xk_mode?Math.round(this.data.width * this.data.export_scale):destWidth,
          height: xk_mode?Math.round(this.data.height * this.data.export_scale):destHeight,
    ...
    imgReset() {
      this.setData({
        scale: 1,
        angle: 0,
        _img_top: (wx.getSystemInfoSync().windowHeight - Math.round(376 / 750 * this.data.info.windowWidth)) / 2,
